...
class Dendrogram(object):
    ...
    def run(self):
        ...
        if isinstance(self._obj, Tree):
            ...
            self._generate_dendro(self._obj, self._rectangles, spacing)
            ...
            for neurite in self._obj.neurites:          
                ...
                self._generate_dendro(neurite, self._rectangles, spacing, offsets)
    ...
    def _generate_dendro(self, current_node, lines, spacing, offsets):
        ...
        for child in current_node.children:
            ...
            lines[self._n] = _vertical_segment(offsets, new_offsets, spacing, radii)
            ...
            self._generate_dendro(child, lines, spacing, new_offsets)
            ...
            if offsets[0] != new_offsets[0]:
                ...
                # horizontal segment
                lines[self._n] = _horizontal_segment(offsets, new_offsets, spacing, 0.)
    ...
    def view(self, new_fig=True, subplot=None, **kwargs):
        ...
        for i, group in enumerate(self._trees):
            ...
            if color not in colors:
                ax.plot((0., 0.), (0., 0.), c=color, label=str(tree_type).replace('TreeType.','').replace('_',' ').capitalize())
                colors.add(color)
